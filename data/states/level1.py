...
class Level1(tools._State):
    ...
    def all_positions(self):
        print("MARIO: X: " + str(self.mario.rect.x) + " Y: " + str(self.mario.rect.bottom))
        green = (0, 255, 0) 

        for posEnemy in self.enemy_group:
            if(posEnemy.rect.x > self.mario.rect.x):
                print("ENEMY: X: " + str(posEnemy.rect.x) + " Y: " + str(posEnemy.rect.bottom))

        for posPowerUp in self.powerup_group:
            if(posPowerUp.rect.x > self.mario.rect.x):
                print("MUSHROOMS: X: " + str(posPowerUp.rect.x) + " Y: " + str(posPowerUp.rect.bottom))
        
        for coinGroup in self.coin_box_group:
            if(coinGroup.rect.x > self.mario.rect.x and abs(coinGroup.rect.x-self.mario.rect.x) < 500):
                print("COIN: X: " + str(coinGroup.rect.x) + " Y: " + str(coinGroup.rect.bottom))
